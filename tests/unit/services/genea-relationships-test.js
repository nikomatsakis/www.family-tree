...
import GeneaService, { Person, Partnership, Relationship } from 'family-tree/services/genea';
import { createMockFamily, createComplexMockFamily } from '../../helpers/mock-genea-data';
...
class MockGeneaService extends Service {
  ...
  _setupData() {
    ...
    for (const [id, data] of Object.entries(this.mockData.people)) {
      this.people[id] = new Person(this, id, data.attributes, data.relationships);
    }
    
    for (const [id, data] of Object.entries(this.mockData.partnerships)) {
      this.partnerships[id] = new Partnership(this, id, data.attributes, data.relationships);
    }
  }
...
module('Unit | Service | genea - Relationship Calculations', function(hooks) {
  ...
  test('calculates sibling relationships correctly', function(assert) {
    ...
    assert.ok(relationship instanceof Relationship, 'Should return a Relationship instance');
    ...
    assert.ok(
      relationshipName.includes('sister') || relationshipName.includes('sibling'),
  ...
  test('calculates cousin relationships through shared grandparents', function(assert) {
    ...
    // Assertions
    assert.ok(relationships.length >= 1, 'Should find at least one relationship path');
  ...
  test('handles complex multi-generation relationships', function(assert) {
    ...
    assert.ok(relationships.length >= 1, 'Should find relationship from child to great-grandparent');
    
    const relationshipName = relationships[0].name;
    assert.ok(
      relationshipName.includes('great') && relationshipName.includes('grand'),
      `Should be great-grandparent relationship: got "${relationshipName}"`
    );
  });
  ...
  test('handles self-relationships appropriately', function(assert) {
    ...
    try {
      ...
      assert.strictEqual(relationships.length, 0, 'Should handle self-relationship gracefully');
    } catch (e) {
  ...
  test('calculates uncle/aunt and nephew/niece relationships', function(assert) {
    ...
    assert.ok(relationships.length >= 1, 'Should find relationship between child and aunt');
    
    const relationshipName = relationships[0].name;
    assert.ok(
      relationshipName.includes('aunt') || relationshipName.includes('pibling'),
  ...
  test('calculates second cousin relationships', function(assert) {
    ...
    assert.ok(relationships.length >= 1, 'Should find relationship between second cousins');
