...
module('Unit | Utils | fuzzy-search', function () {
  ...
  module('Middle Name Skipping', function () {
    test('finds Leonardo da Vinci by partial first and last name', function (assert) {
      ...
      assert.strictEqual(
        results[0].name,
        'Leonardo Wilhelm da Vinci',
        'Should find Leonardo da Vinci',
      );
    });
    ...
    test('finds Marie Curie by skipping middle name', function (assert) {
      ...
      assert.strictEqual(
        ...
        'Should find Marie Curie despite middle name',
      );
    });
    ...
    test('finds Benjamin Roosevelt by partial names', function (assert) {
      ...
      assert.strictEqual(
        ...
        'Should find Benjamin Roosevelt',
      );
    });
    ...
    test('finds Mozart by partial first and middle name', function (assert) {
      ...
      assert.strictEqual(
        ...
        'Should find Mozart by partial names',
      );
    });
    ...
    test('finds Queen Elizabeth by first and last name', function (assert) {
      ...
      assert.strictEqual(
        results[0].name,
        'Elizabeth Alexandra Windsor',
        'Should find Queen Elizabeth',
      );
    });
  });
  ...
  module('Partial First and Last Name Matching', function () {
    ...
    test('finds Martin Luther King with partial first and last name', function (assert) {
      ...
      console.log(
        'Debug - Search results for "Mar King":',
  ...
  module('Typo Tolerance', function () {
    test('finds Leonardo with typo in name', function (assert) {
      ...
      assert.strictEqual(
        results[0].name,
        'Leonardo Wilhelm da Vinci',
        'Should find Leonardo despite typo',
      );
    });
    ...
    test('finds Mozart with typo in middle name', function (assert) {
      ...
      assert.strictEqual(
        results[0].name,
        'Wolfgang Amadeus Mozart',
        'Should find Mozart despite typo',
      );
    });
  });
  ...
  module('Word Order Independence', function () {
    test('finds results regardless of word order', function (assert) {
      ...
      assert.strictEqual(
        ...
        1,
        'Forward order should find result',
      );
      assert.strictEqual(
        reverseResults.length,
        1,
        'Reverse order should find result',
      );
      assert.strictEqual(
    ...
    test('finds Leonardo with words in different order', function (assert) {
      ...
      assert.strictEqual(
        results.length,
        1,
        'Should find result with reversed words',
      );
      assert.strictEqual(
        results[0].name,
        'Leonardo Wilhelm da Vinci',
        'Should find Leonardo',
      );
    });
  });
  ...
  module('Comments Field Search', function () {
    test('finds person by searching comments field', function (assert) {
      ...
      assert.strictEqual(
        results[0].name,
        'Albert Einstein',
        'Should find Einstein by comment content',
      );
    });
    ...
    test('finds person by name and comment combination', function (assert) {
      ...
      assert.strictEqual(
        results.length,
        1,
        'Should find result combining name and comment',
      );
      assert.strictEqual(
        results[0].name,
        'Marie Skłodowska Curie',
  ...
  module('Edge Cases', function () {
    ...
    test('handles single character search', function (assert) {
      ...
      assert.ok(
        results.length > 0,
        'Single character should find some results',
      );
      assert.ok(
