...
import {
  RenderTreeBuilder,
  createDefaultRenderTree,
} from 'family-tree/utils/render-tree-builder';
...
module('Unit | Utils | render-tree-builder', function (hooks) {
  ...
  test('RenderTreeBuilder exists and can be constructed', function (assert) {
    ...
    assert.strictEqual(
      builder.renderTree.persons.length,
      0,
  ...
  test('addPrimaryPerson adds person and their marriages', function (assert) {
    ...
    assert.notStrictEqual(momIndex, undefined, 'Mom was added');
  ...
  test('createDefaultRenderTree builds minimal family view', function (assert) {
    ...
    assert.strictEqual(
      renderTree.focusPersonIndex,
      0,
      'Focus person set correctly',
    );
    ...
    assert.strictEqual(
      focusPerson.name,
      'Child One',
      'Focus person is Child One',
    );

    // Should have: Child One, Dad (primary parent), Mom (partner), and Child Two (sibling)
    assert.strictEqual(
      renderTree.persons.length,
      4,
      'Has 4 people: focus + parents + siblings',
    );
    ...
    assert.notStrictEqual(dadIndex, -1, 'Dad found in tree');
    assert.notStrictEqual(momIndex, -1, 'Mom found in tree');
  ...
  test('createDefaultRenderTree with person who has parents shows 3 generations', function (assert) {
    ...
    assert.ok(
      renderTree.persons.length >= 4,
      'Has at least 4 people including grandparents',
    );

    // Check that Grandpa was added as primary lineage
    const grandpaIndex = renderTree.findPersonByOriginalId('gp1');
    assert.notStrictEqual(grandpaIndex, -1, 'Grandpa found in tree');
  ...
  test('debug output is readable', function (assert) {
    ...
    assert.strictEqual(
      debug.focusPersonName,
      'Child One',
      'Focus person name correct',
    );
